...
METHOD=snow
bash original_scripts/image_clip_bart_corr_hp/single_adapter_reduction_factor_16.sh $((PORT+9)) \
  --img_corruption_method ${METHOD} \
  --img_corruption_severity ${SEVERITY} \
  --gpu_number ${GPU_NUMBER} \
  &> test_on_corrupted_images_single_adapter_reduction_factor_16_${METHOD}_${SEVERITY}.out
...
METHOD=frost
bash original_scripts/image_clip_bart_corr_hp/single_adapter_reduction_factor_16.sh $((PORT+10)) \
  --img_corruption_method ${METHOD} \
  --img_corruption_severity ${SEVERITY} \
  --gpu_number ${GPU_NUMBER} \
  &> test_on_corrupted_images_single_adapter_reduction_factor_16_${METHOD}_${SEVERITY}.out
