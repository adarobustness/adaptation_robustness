...
METHOD=gaussian_blur
bash original_scripts/image_clip_bart_corr_subset/single_compacter.sh 21787 \
  --img_corruption_method ${METHOD} \
  --img_corruption_severity ${SEVERITY} \
  --gpu_number ${GPU_NUMBER} \
  --subset_ratio ${RATIO} \
...
METHOD=frost
bash original_scripts/image_clip_bart_corr_subset/single_compacter.sh $((PORT+10)) \
  --img_corruption_method ${METHOD} \
  --img_corruption_severity ${SEVERITY} \
  --gpu_number ${GPU_NUMBER} \
  --subset_ratio ${RATIO} \
