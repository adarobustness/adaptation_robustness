...
PORT=$3
GPU_NUMBER=$1
#bash original_scripts/image_clip_t5_corr_test/single_adapter.sh $((PORT)) \
#  --img_corruption_method none \
#  --img_corruption_severity 1 \
#  --gpu_number ${GPU_NUMBER} \
#  &> test_on_corrupted_images_single_adapter_none_1_t5.out

# try out
#bash original_scripts/image_clip_t5_corr_test/single_adapter.sh 26787 \
...
METHOD=shot_noise
bash original_scripts/image_clip_t5_corr_test/single_adapter.sh $((PORT+2)) \
  --img_corruption_method ${METHOD} \
  --img_corruption_severity ${SEVERITY} \
  --gpu_number ${GPU_NUMBER} \
  &> test_on_corrupted_images_single_adapter_${METHOD}_${SEVERITY}_t5.out
