...
GPU_NUMBER=$1
METHOD=drop_first
bash original_scripts/image_clip_t5_corr_test/single_adapter.sh $((PORT+16)) \
...
METHOD=drop_last
bash original_scripts/image_clip_t5_corr_test/single_adapter.sh $((PORT+17)) \
  --text_corruption_method ${METHOD} \
  --text_corruption_severity ${SEVERITY} \
  --gpu_number ${GPU_NUMBER} \
  &> test_on_corrupted_text_single_adapter_${METHOD}_${SEVERITY}_t5.out
...
bash original_scripts/image_clip_t5_corr_test/single_adapter.sh $((PORT+18)) \
  --text_corruption_method ${METHOD} \
  --text_corruption_severity ${SEVERITY} \
  --gpu_number ${GPU_NUMBER} \
  &> test_on_corrupted_text_single_adapter_${METHOD}_${SEVERITY}_t5.out
