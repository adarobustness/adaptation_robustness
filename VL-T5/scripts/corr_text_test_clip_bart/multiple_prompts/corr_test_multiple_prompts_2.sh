...
GPU_NUMBER=$1
METHOD=drop_first
bash original_scripts/image_clip_bart_corr_test/multiple_prompts.sh $((PORT+16)) \
...
METHOD=drop_last
bash original_scripts/image_clip_bart_corr_test/multiple_prompts.sh $((PORT+17)) \
  --text_corruption_method ${METHOD} \
  --text_corruption_severity ${SEVERITY} \
  --gpu_number ${GPU_NUMBER} \
  &> test_on_corrupted_text_multiple_prompts_${METHOD}_${SEVERITY}.out
...
bash original_scripts/image_clip_bart_corr_test/multiple_prompts.sh $((PORT+18)) \
  --text_corruption_method ${METHOD} \
  --text_corruption_severity ${SEVERITY} \
  --gpu_number ${GPU_NUMBER} \
  &> test_on_corrupted_text_multiple_prompts_${METHOD}_${SEVERITY}.out
