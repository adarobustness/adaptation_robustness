...
class Trainer(TrainerBase):
    def __init__(
        ...
        from classification_model import VLT5Classification, VLBartClassification

        model_kwargs = {}
        if "t5" in args.backbone:
            model_class = VLT5Classification
        elif "bart" in args.backbone:
            model_class = VLBartClassification
    ...
    def train(self):
        if self.verbose:
            ...
            if "t5" in self.args.backbone:
                if self.args.use_vision:
                    project_name = "VLT5_Classification"
                else:
                    project_name = "T5_Classification"
            elif "bart" in self.args.backbone:
                if self.args.use_vision:
                    project_name = "VLBart_Classification"
                else:
                    project_name = "Bart_Classification"
        ...
        for epoch in range(self.args.epochs):
            ...
            if self.verbose:
                ...
                log_str = ""
                log_str += "\nEpoch %d: Valid Top1 %0.2f" % (epoch, valid_score_raw)
        ...
        if self.verbose:
            score_dict = {
                "overall": self.compute_acc(quesid2ans, self.test_loader.gts_for_data)
            }
            wandb_log_dict = {}
            wandb_log_dict["Test/overall"] = score_dict["overall"]
    ...
    def compute_acc(self, quesid2ans, gts_for_data):
        correct = 0
        i = 0
    ...
    def evaluate(self, loader, dump_path=None):
        ...
        if self.verbose:
            ...
            acc_dict = {"overall": self.compute_acc(quesid2ans, gts_for_data)}
...
def main_worker(gpu, args):
    ...
    if args.feature_type == "butd":
        from classification_data import get_loader
        ...
        from classification_raw_data import get_loader
    else:
        feat_dim_dict = {"RN50": 2048, "RN101": 2048, "RN50x4": 2560, "ViT": 768}
        args.feat_dim = feat_dim_dict[args.feature_type]
        from classification_clip_data import get_loader
