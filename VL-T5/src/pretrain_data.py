...
def get_datum(datum):
    ...
    for text_source, sents in datum["sentf"].items():
        ...
        for sent_idx, sent in enumerate(sents):
            if ("t5" in datum["backbone"] or "bart" in datum["backbone"]) and len(
                sent.split()
            ) <= 2:
                continue
...
class PretrainDataset(Dataset):
    def __init__(
        self, split="vg", rank=-1, topk=-1, verbose=True, args=None, is_train=True
    ):
        self.topk = topk
        ...
        if self.verbose:
            print(
                "Load an answer table of size %d."
        ...
        with Pool(8) as pool:
            if self.verbose:
                data = [
                    datum
                    for _data in tqdm(
                        pool.imap(get_datum, data),
                        total=len(data),
                        ncols=100,
                        desc="Creating pretrainig data examples",
                    )
                    for datum in _data
                ]
            else:
                data = [
                    datum for _data in pool.imap(get_datum, data) for datum in _data
                ]
        ...
        if self.args.itm_cocoonly:
            ...
            caption_sources = ["mscoco", "vg"]
        self.data_captions = [
            datum for datum in data if datum["text_source"] in caption_sources
        ]
        ...
        self.source_to_h5 = {
            "mscoco_resplit_train_train2014": coco_dir.joinpath("features").joinpath(
                "train2014_obj36.h5"
            ),
            "mscoco_resplit_train_val2014": coco_dir.joinpath("features").joinpath(
                "val2014_obj36.h5"
            ),
            "mscoco_resplit_val": coco_dir.joinpath("features").joinpath(
                "resplit_val_obj36.h5"
            ),
