...
class GQAFineTuneDataset(Dataset):
    def __init__(
            ...
            if source == "testdev":
                if args.text_corruption_method == "none":
                    ...
                    data_info_path = dataset_dir.joinpath(
                        f"GQA/{source}_corrupted_{args.text_corruption_method}_{args.text_corruption_severity}.json"
                    )
            else:
                data_info_path = dataset_dir.joinpath(f"GQA/{source}.json")
        ...
        self._update_corrupted_featname_to_h5(
            self.args.img_corruption_method, self.args.img_corruption_severity
        )

    def _update_corrupted_featname_to_h5(self, method: str, severity: int):
        if method == "none":
            return
        ...
        if "label" in datum:
            ...
            if sum(scores) > 0:
                ...
                out_dict["best_answers_tokenized"] = [
                    self.tokenizer.encode(a) for a in best_answers
                ]
            else:
                out_dict["best_answers_tokenized"] = [[]]
...
class GQAEvaluator:
    ...
    def evaluate(self, quesid2ans: dict):
        ...
        for quesid, ans in quesid2ans.items():
            ...
            if ans in label:
                score += label[ans]
        return {"Accuracy": score / len(quesid2ans)}
