...
class LoraBartAttention(BartAttention):
    ...
    def __init__(
        ...
        self.k_proj = nn.Linear(embed_dim, embed_dim, bias=bias)
        self.v_proj = lora.LoRALinearController(
            embed_dim, embed_dim, config=config, bias=bias
        )
        self.q_proj = lora.LoRALinearController(
            embed_dim, embed_dim, config=config, bias=bias
        )
        self.out_proj = nn.Linear(embed_dim, embed_dim, bias=bias)
...
class BartEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        if self.adapter_hypernet is not None:
            hidden_states = self.adapter_hypernet(
                hidden_states, block_adapters.self_attention
            )
        ...
        if self.adapter_hypernet is not None:
            hidden_states = self.adapter_hypernet(
                hidden_states, block_adapters.feed_forward
            )
...
class BartDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            if (
                past_key_value is not None and len(past_key_value) == 4
            ):  # len(past_key_value) is for prefix
            ...
            if self.adapter_hypernet is not None and self.add_adapter_cross_attn:
                hidden_states = self.adapter_hypernet(
                    hidden_states, block_adapters.cross_attention
                )
        ...
        if config.use_hyperformer:
            if config.adapter_config.unique_hyper_net:
                self.adapter_layers_hyper_net = AdapterLayersHyperNetController(
                    config.adapter_config, config.encoder_layers
                )
...
class BartDecoder(BartPretrainedModel):
    ...
    def __init__(
        ...
        if config.use_hyperformer:
            if config.adapter_config.unique_hyper_net:
                self.adapter_layers_hyper_net = AdapterLayersHyperNetController(
                    config.adapter_config, config.decoder_layers, True
                )
            if config.adapter_config.efficient_unique_hyper_net:
                self.adapter_layers_hyper_net = AdapterLayersOneHyperNetController(
                    config.adapter_config, config.decoder_layers, True
...
class BartModel(BartPretrainedModel):
    ...
    @add_code_sample_docstrings(
        #tokenizer_class=_TOKENIZER_FOR_DOC, # bug said this is an unexpected keyword argument
...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    def adjust_logits_during_generation(self, logits, cur_len, max_length=20):
