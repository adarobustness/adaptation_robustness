...
class T5LayerFF(nn.Module):
    ...
    def forward(self, hidden_states, block_adapters, task):
        ...
        if self.adapter_hypernet is not None:
            forwarded_states = self.adapter_hypernet(
                forwarded_states, block_adapters.feed_forward
            )
...
class T5LayerCrossAttention(nn.Module):
    def __init__(self, config):
        ...
        if (
            config.use_adapter or config.use_compacter or config.use_lradapter
        ) and config.add_adapter_cross_attn:
        ...
        if config.use_hyperformer:
            if config.adapter_config.unique_hyper_net:
                self.adapter_layers_hyper_net = AdapterLayersHyperNetController(
                    config.adapter_config, config.num_layers, config.is_decoder
                )
