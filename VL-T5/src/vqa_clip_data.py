...
class VQAFineTuneDataset(Dataset):
    def __init__(
        ...
        for source in self.sources:
            if source == "karpathy_test":
                if args.text_corruption_method == "none":
                    data_info_path = dataset_dir.joinpath(f"vqa/{source}.json")
                else:
                    data_info_path = dataset_dir.joinpath(
                        f"vqa/{source}_corrupted_{args.text_corruption_method}_{args.text_corruption_severity}.json"
                    )
            else:
                data_info_path = dataset_dir.joinpath(f"vqa/{source}.json")
        ...
        if isinstance(self.topk, float) and (0 < self.topk <= 1):
            ...
            if self.verbose:
                logger.info(f"Use only {len(data)} data given topk={self.topk} and total {original_len} data")

        elif self.topk > 0:
            data = data[: int(self.topk)]
            if self.verbose:
                logger.info(f"Use only {len(data)} data")
        ...
        self._update_corrupted_source_to_h5(
            self.args.img_corruption_method, self.args.img_corruption_severity
        )

        self.h5_path = self.source_to_h5[self.feature_type]

    def _update_corrupted_source_to_h5(self, method: str, severity: int):
        if method == "none":
            return
        assert method in IMG_CORRUPTION_METHODS, (
            f"method should be one of {IMG_CORRUPTION_METHODS}, but got {method}"
        )
        assert 1 <= severity <= 5 or severity == 100, (
            f"severity should be in [1, 5] or 100 (for debugging), but got {severity}"
        )
        self.source_to_h5.update(
            {
                _type: coco_feature_dir.joinpath(f"data_clip_{self.args.feature_type}_att_test_{method}_{severity}/data_clip_{self.args.feature_type}_att/")
                for _type in ["RN50", "RN101", "RN50x4", "ViT"]
            }
        )
    ...
    def __getitem__(self, idx):
        ...
        if "sent" in datum:
            ...
            sent = datum["question"]
        # if "karpathy_test" in self.sources:
        #     logger.info(f"self.sources {self.sources}")
        #     logger.info(f"sent: {sent}")
        #     assert False
        # decode input texts to ids
        input_ids = self.tokenizer.encode(
            f"{self.args.prompt}{sent}{self.args.post_prompt}",
            max_length=20,
            truncation=True,
        )
        ...
        if "label" in datum:
            ...
            if self.args.classifier:
                ...
                target_ids = self.tokenizer.encode(
                    answer, max_length=10, truncation=True
                )
...
def get_loader(
    ...
    _dset = VQADataset(split, args, verbose)
...
class VQADataset:
    ...
    def __init__(self, splits: str, args, verbose=True):
        self.name = splits
        ...
        for split in self.splits:
            # actually, we didn't modify the input sent here, the real loading is done in VQAFineTuneDataset.__init__.
            if split == "karpathy_test":
                if args.text_corruption_method == "none":
                    self.data.extend(json.load(open(vqa_dir.joinpath(f"{split}.json"))))
                else:
                    self.data.extend(
                        json.load(
                            open(
                                vqa_dir.joinpath(
                                    f"karpathy_test_corrupted_{args.text_corruption_method}_{args.text_corruption_severity}.json"
                                )
                            )
                        )
                    )
