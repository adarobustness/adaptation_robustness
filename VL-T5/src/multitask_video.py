    ...
    def train(self):
        ...
        if self.args.epochs == 0:  # test only
            ...
            if "tvqa" in self.args.tasks:
                ...
                with open(
                    os.path.join(self.args.output, "tvqa_test_predictions.json"), "w"
                ) as f:
            ...
            if "how2qa" in self.args.tasks:
                ...
                with open(
                    os.path.join(
                        self.args.output, "how2qa_test_public_predictions.json"
                    ),
                    "w",
                ) as f:
                    json.dump(dump_results, f)
            ...
            if "tvc" in self.args.tasks:
                ...
                with open(
                    os.path.join(self.args.output, "tvc_test_predictions.jsonl"), "w"
                ) as f:
            ...
            if "yc2c" in self.args.tasks:
                ...
                with open(
                    os.path.join(self.args.output, "yc2c_test_predictions.jsonl"), "w"
                ) as f:
                    for entry in dump_results:
                        json.dump(entry, f)
                        f.write("\n")
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(tqdm(loader, ncols=150)):
                ...
                for pred, ques in zip(pred_ans, ques_ids):
                    quesid2ans.append({"question_id": ques, "answer": pred})
...
def main_worker(gpu, args):
    ...
    if args.epochs > 0:
        if "tvqa" in args.tasks:
            ...
            tvqa_train_loader = tvqa_data.get_loader(
                ...
                split="train",
                mode="train",
                batch_size=tvqa_args.batch_size,
        ...
        if "how2qa" in args.tasks:
            ...
            how2qa_train_loader = how2qa_data.get_loader(
                ...
                split="train_release",
                mode="train",
                batch_size=how2qa_args.batch_size,
                distributed=args.distributed,
                gpu=args.gpu,
                workers=args.num_workers,
                topk=args.train_topk,
        ...
        if "tvc" in args.tasks:
            ...
            tvc_train_loader = tvc_data.get_loader(
                ...
                split="train_release",
                mode="train",
                batch_size=tvc_args.batch_size,
                distributed=args.distributed,
                gpu=args.gpu,
                workers=args.num_workers,
                topk=args.train_topk,
        ...
        if "yc2c" in args.tasks:
            ...
            yc2c_train_loader = yc2c_data.get_loader(
                ...
                split="train_release",
                mode="train",
                batch_size=yc2c_args.batch_size,
                distributed=args.distributed,
                gpu=args.gpu,
                workers=args.num_workers,
                topk=args.train_topk,
    ...
    train_loader = multitask_data.MultitaskLoader(
        train_loaders, sampling=args.multitask_sampling, verbose=gpu == 0
    )
            ...
            if args.epochs == 0:  # test only
                if "tvqa" in args.tasks:
                    ...
                    tvqa_test_loader = tvqa_data.get_loader(
                        tvqa_args,
                        split="test_public,test_release",
                        mode="val",
                        batch_size=tvqa_args.batch_size,
                        distributed=False,
                        gpu=args.gpu,
