...
class TrainerBase(object):
    ...
    def create_config(self):
        ...
        config.additional_visual_embedding_layers = (
            args.additional_visual_embedding_layers
        )
        ...
        config.max_length = args.max_text_length

        tasks = re.split("[, ]+", args.tasks)  # tranform to list

        if (
            args.use_hyperformer
            or args.use_adapter
            or args.use_compacter
            or args.use_lradapter
        ):
            assert (
                config.use_hyperformer
                + config.use_adapter
                + config.use_compacter
                + config.use_lradapter
                <= 1
            ...
            config.adapter_config.efficient_unique_hyper_net = (
                args.efficient_unique_hyper_net
            )
            ...
            config.adapter_config.shared_phm_rule_over_tasks = (
                args.shared_phm_rule_over_tasks
            )

            config.adapter_config.add_layer_norm_before_adapter = (
                args.add_layer_norm_before_adapter
            )
            config.adapter_config.add_layer_norm_after_adapter = (
                args.add_layer_norm_after_adapter
            )

            config.adapter_config.track_z = args.track_z

            if args.projected_task_embedding_dim != -1:
                config.adapter_config.projected_task_embedding_dim = (
                    args.projected_task_embedding_dim
                )
        else:
        ...
        if args.encoder_prompt_len > 0:
            ...
            config.encoder_prompt_config.position = args.encoder_prompt_position
            config.encoder_prompt_config.init_method = args.encoder_prompt_init_method
        else:
    ...
    def create_model(self, model_class, config=None, **kwargs):
        logger.critical(f"Building Model at GPU {self.args.gpu}")
        ...
        def LM_LN_eval(model):
            for name, sub_module in model.named_modules():
                ...
                if isinstance(
                    sub_module,
                    (
                        modeling_bart.JointEncoder,
                        modeling_bart.BartDecoder,
                        modeling_t5.T5Stack,
    ...
    def unfreeze_parameters(self):
        ...
        for name, sub_module in self.model.named_modules():
            ...
            if (
                self.args.use_adapter
                or self.args.use_compacter
                or self.args.use_lradapter
            ):
                    ...
                    vis_optimizer_grouped_parameters = [
                        {
                            "params": [
                                p
                                for n, p in visn_model.named_parameters()
