...
class TVRFineTuneDataset(Dataset):
    def __init__(
        self,
        split="train",
        raw_dataset=None,
        ...
        annotations = dataset_dir.joinpath(f"ann/tvr/tvr_{self.source}_release.jsonl")
    ...
    def __getitem__(self, idx):
        ...
        sent = f'{datum["desc"].strip()} Duration: {datum["duration"]:.2f}.'

        input_ids = self.tokenizer.encode(
            f"{sent}. {self.args.prompt}", max_length=30, truncation=True
        )

        out_dict["question_id"] = datum["desc_id"]
...
class ActivityNetQAEvaluator:
    ...
    def output(self, corrects, type_count):
        ...
        free_type_accuracy = free_type_corrects_count / (
            float(sum(list(type_count.values())[3:])) + 1e-10
        )
