...
class VLT5Video(VLT5):
    ...
    def train_step(self, batch):
        ...
        if self.config.classifier:
            ...
            last_hidden_state = last_layer_hidden_state.view(
                B, -1, self.config.d_model
            )[:, -1]
    ...
    def test_step(self, batch, **kwargs):
        ...
        if self.config.classifier:
            ...
            logits = (
                ...
                .detach()
                .cpu()
                .view(B, -1, self.lm_head.out_features)[:, 1]
            )
            logits = logits.view(B, self.lm_head.out_features)
            true_logit = logits[:, self.true_id].view(B, 1)
            false_logit = logits[:, self.false_id].view(B, 1)

            # (B, 2)
            binary_logits = torch.cat([true_logit, false_logit], dim=1)

            confidence = torch.softmax(binary_logits, dim=1)[
                :, 0
            ]  # The probability of true
            confidence = confidence.view(B // 5, 5)
            confidence = confidence / confidence.sum(dim=1, keepdim=True)

            prediction = (
                torch.argmax(confidence, dim=1).numpy().tolist()
            )  # list of indices

            generated_sents = [f"a{p}" for p in prediction]

            result["token_ids"] = prediction
            result["pred_ans"] = generated_sents
