...
class TVQAFineTuneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(5):
            ...
            input_ids = self.tokenizer.encode(
                f"{sent} {self.args.prompt}", max_length=30, truncation=True
            )
            ...
            if "answer" in datum:
                ...
                target_ids = self.tokenizer.encode(
                    out_dict[f"{answer_id}/answer"], max_length=20, truncation=True
                )
    ...
    def collate_fn(self, batch):
        ...
        input_ids = (
            torch.ones(B * 5, S_W_L, dtype=torch.long) * self.tokenizer.pad_token_id
        )
        ...
        if "a0/target_ids" in batch[0]:
            T_W_L = max(
                entry[f"a{i}/target_length"] for entry in batch for i in range(5)
            )
            target_ids = (
                torch.ones(B * 5, T_W_L, dtype=torch.long) * self.tokenizer.pad_token_id
            )
        ...
        for i, entry in enumerate(batch):
            for j in range(5):
                input_ids[i * 5 + j, : entry[f"a{j}/input_length"]] = entry[
                    f"a{j}/input_ids"
                ]
                ...
                if f"a{j}/target_ids" in entry:
                    target_ids[i * 5 + j, : entry[f"a{j}/target_length"]] = entry[
                        f"a{j}/target_ids"
                    ]
...
class TVQAEvaluator:
    ...
    def output(self, corrects, type_count):
        ...
        all_type_accuracy = all_type_corrects_count / (
            float(sum(type_count.values())) + 1e-10
        )
