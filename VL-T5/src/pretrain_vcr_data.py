...
class VCRPretrainDataset(Dataset):
    def __init__(
        self, split="train", rank=-1, topk=-1, verbose=True, args=None, is_train=True
    ):
        self.topk = topk
        ...
        with Pool(8) as pool:
            if self.verbose:
                data = [
                    datum
                    for _data in tqdm(
                        pool.imap(get_datum, data_info_dicts),
        ...
        if (
            self.verbose
            and is_train
            and ("t5" in self.args.backbone or "bart" in self.args.backbone)
        ):
        ...
        if "t5" in self.args.backbone or "bart" in self.args.backbone:
            ...
            if task == "lm":
                ...
                if datum["vcr_task"] == "QA":
                    ...
                    source_text = (
                        f"rationale prediction: question: {flat_q} answer: {flat_a}"
                    )
                    target_text = flat_r
