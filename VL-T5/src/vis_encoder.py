...
class CLIPResNetEncoder(nn.Module):
    def __init__(
        ...
        pos_embed.weight = resize_pos_embed(
            self.model.visual.attnpool.positional_embedding.unsqueeze(0), pos_embed
        )
    ...
    def reset_image_size(self, image_size=224, patch_size=32):
        ...
        previous_position_embedding_weight = (
            self.model.visual.attnpool.positional_embedding.data[0, 1:]
        )
        previous_position_embedding_weight = (
            previous_position_embedding_weight.transpose(1, 0)
        )
        previous_position_embedding_weight = previous_position_embedding_weight.view(
            1, self.config.hidden_size, old_grid, old_grid
        )
        ...
        new_position_embedding_weight = new_position_embedding_weight.view(
            self.config.hidden_size, new_grid**2
        ).transpose(1, 0)

        new_position_embedding = nn.Parameter(
            torch.zeros(1 + new_grid**2, self.config.hidden_size)
        )
        # [CLS]
        new_position_embedding.data[
            0
        ] = self.model.visual.attnpool.positional_embedding.data[0, 0]
        new_position_embedding.data[1:] = new_position_embedding_weight

        self.model.visual.attnpool.positional_embedding = nn.Parameter(
            new_position_embedding.unsqueeze(0)
        )
...
if __name__ == "__main__":
    ...
    clip_model = get_vis_encoder(
        "RN50x4", adapter_type="front/middle/back-bottleneck", reduction_factor=26
    )
