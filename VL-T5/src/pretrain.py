        ...
        for epoch in range(self.args.epochs):
                ...
                if self.verbose:
                    ...
                    for i, (loss_name, loss_meter) in enumerate(
                        ...
                        if loss_name in results:
                            loss_meter.update(
                                results[f"{loss_name}"] / results[f"{loss_name}_count"]
                            )
                        if len(loss_meter) > 0:
                            loss_count = epoch_results[f"{loss_name}_count"]
                            desc_str += (
                                f" {loss_name} ({loss_count}) {loss_meter.val:.3f}"
                            )
            ...
            if "qa" in self.args.losses:
                (
                    ...
                    score,
                    cnt,
                ) = self.val_loader.dataset.evaluator.evaluate(valid_uid2ans)
                ...
                score_cnt_dict = reduce_dict(
                    {"score": score, "cnt": cnt}, average=False
                )
