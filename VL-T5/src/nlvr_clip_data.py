...
class NLVRFineTuneDataset(Dataset):
    def __init__(
        ...
        self.source_to_h5 = {
            "train": nlvr_dir.joinpath(
                f"clip_features/data_clip_{self.args.feature_type}_att/"
            ),
            "valid": nlvr_dir.joinpath(
                f"clip_features/data_clip_{self.args.feature_type}_att/"
            ),
            "test": nlvr_dir.joinpath(
                f"clip_features/data_clip_{self.args.feature_type}_att/" 
            ),
        }

        self._update_corrupted_source_to_h5(self.args.img_corruption_method, self.args.img_corruption_severity)

    def _update_corrupted_source_to_h5(self, method: str, severity: int):
        """
        method: corruption method name
        severity: 1-5
        return: updated source_to_h5 with test directed to corrupted features
        """
        if method == "none":
            return self.source_to_h5
        assert method in IMG_CORRUPTION_METHODS, (
            f"method should be one of {IMG_CORRUPTION_METHODS}, but got {method}"
        )
        assert 1 <= severity <= 5 or severity == 100, f"severity should be in [1, 5] or 100 (for debugging), but got {severity}"
        self.source_to_h5.update(
            {
                "test": nlvr_dir.joinpath(
                    f"clip_features/data_clip_{self.args.feature_type}_att_test_{method}_{severity}/data_clip_{self.args.feature_type}_att/"
                )
            }
        )
...
def get_loader(
    ...
    _dset = NLVR2Dataset(split, args, verbose)
        ...
        for split in self.splits:
            if split == "test":
                if args.text_corruption_method == "none":
                    self.data.extend(json.load(open(nlvr_dir.joinpath(f"{split}.json"))))
                else:
                    self.data.extend(
                        json.load(
                            open(
                                nlvr_dir.joinpath(
                                    f"test_corrupted_{args.text_corruption_method}_{args.text_corruption_severity}.json"
                                )
                            )
                        )
                    )
