...
from my_transformers.modeling_bart import (
    BartModel,
...
class VisualEmbedding(nn.Module):
    def __init__(self, config, obj_order_embedding):
        ...
        if self.vis_use_transformer:
            ...
            feat_embedding.append(CustomForward(transformer_model))
...
class ExpandVisualEmbedding(nn.Module):
    def __init__(self, config, obj_order_embedding):
        ...
        absolute_vis_pos_embedding = [
            nn.Linear(pos_dim + 1, n_image_tokens * config.d_model)
        ]

        # use custom layer norm
        if self.config.use_vis_layer_norm and self.config.individual_vis_layer_norm:
            absolute_vis_pos_embedding.append(
                nn.LayerNorm(n_image_tokens * config.d_model)
            )
        self.absolute_vis_pos_embedding = nn.Sequential(*absolute_vis_pos_embedding)

        if self.config.use_vis_order_embedding:
            self.obj_order_embedding = nn.Embedding(
                config.n_boxes, n_image_tokens * config.d_model
            )
            self.img_order_embedding = nn.Embedding(
                n_images, n_image_tokens * config.d_model
            )
...
class ResidualVisualEmbedding(nn.Module):
    def __init__(self, config, obj_order_embedding):
        ...
        feat_embedding = [
            LinearModule(feat_dim, config.d_model * config.encoder_layers * 4)
        ]

        # use custom layer norm
        if self.config.use_vis_layer_norm and self.config.individual_vis_layer_norm:
            feat_embedding.append(
                nn.LayerNorm(config.d_model * config.encoder_layers * 4)
            )

        self.feat_embedding = nn.Sequential(*feat_embedding)

        absolute_vis_pos_embedding = [
            LinearModule(pos_dim + 1, config.d_model * config.encoder_layers * 4)
        ]
        ...
        if self.config.use_vis_order_embedding:
            ...
            self.obj_order_embedding = nn.Sequential(
                ...
                LinearModule(
                    config.d_model, config.d_model * config.encoder_layers * 4
                ),
            )
        ...
        if self.config.use_vis_layer_norm and not self.config.individual_vis_layer_norm:
            self.layer_norm = nn.LayerNorm(
                config.d_model * config.encoder_layers * 4 * encoder_attention_heads
            )
...
class Downsample(nn.Module):
    ...
    def forward(self, inputs_tuple):
        ...
        if len(inputs_tuple) == 4:  # (NLVR)
            ...
            boxes = boxes[
                :, : inputs.shape[1]
            ]  # Get the first few data because the element are all zeros
...
class JointEncoder(BartEncoder):
    ...
    def __init__(self, config, embed_tokens=None, task_embed=None):
        ...
        if (
            config.expand_vis_embedding
        ):  # This is not use right now, this is for Frozen type visual embeddings
        ...
        # logger.info(f" prompt weights before universal init: {self.prompt_modules.get_prompt('vqa').prefix_embedding[0].weight[0, 0:10]}")
        self.init_weights()
        # logger.info(
        #     f" prompt weights after universal init: {self.prompt_modules.get_prompt('vqa').prefix_embedding[0].weight[0, 0:10]}")

        if config.encoder_prompt_config:
            # Initialize prompt modules
            for task in self.prompt_modules.tasks:
                PROMPT_INIT_METHODS[config.encoder_prompt_config.init_method](
                    self.prompt_modules.get_prompt(task).prefix_embedding[0].weight,
                )
            # logger.info(
            #     f" prompt weights after prompt init: {self.prompt_modules.get_prompt('vqa').prefix_embedding[0].weight[0, 0:10]}")
    ...
    def forward(
        ...
        # Batch size, input length, embedding size, e.g. 416 6 768
        # logger.critical(f"input embeds shape {inputs_embeds.shape}")
        ...
        if self.prompt_modules is not None:
            ...
            # Batch size, prompt length, embedding size, e.g. 416 12 768
            # logger.critical(f"prefix embeds shape {prefix_embeds.shape}")
            if self.config.encoder_prompt_config.position == "front":
                # logger.info("prompt at front")
                inputs_embeds = torch.cat([prefix_embeds, inputs_embeds], dim=1)
            elif self.config.encoder_prompt_config.position == "middle" or self.config.encoder_prompt_config.position == "back":
                # logger.info(f"prompt at {self.config.encoder_prompt_config.position}")
                inputs_embeds = torch.cat([inputs_embeds, prefix_embeds], dim=1)
            # Batch size, input length + prompt length, embedding size, e.g. 416 18 768
            # logger.critical(f"input embeds shape {inputs_embeds.shape} after prompt")
        ...
        if vis_inputs is not None:
            ...
            if self.config.share_vis_lang_layer_norm: # False
                inputs_embeds = torch.cat([inputs_embeds, vis_embeds], dim=1)

                inputs_embeds = self.layernorm_embedding(inputs_embeds)
            else:
                # logger.critical(f"concatenate input with vis embeds")
                inputs_embeds = self.layernorm_embedding(inputs_embeds)
                if self.prompt_modules is not None:
                    if self.config.encoder_prompt_config.position == "front" or \
                            self.config.encoder_prompt_config.position == "middle":
                        inputs_embeds = torch.cat([inputs_embeds, vis_embeds], dim=1)
                    elif self.config.encoder_prompt_config.position == "back":
                        inputs_embeds = torch.cat(
                            [inputs_embeds[:, :input_length, :], vis_embeds, inputs_embeds[:, input_length:, :]],
                            dim=1)
                else:
                    inputs_embeds = torch.cat([inputs_embeds, vis_embeds], dim=1)
                # logger.critical(f"input embeds shape {inputs_embeds.shape} after catting vis embeds")

            if vis_attention_mask is None:
                vis_attention_mask = torch.ones(
                    B, V_L, dtype=inputs_embeds.dtype, device=inputs_embeds.device
                )
        ...
        if vis_attention_mask is not None:
            if self.prompt_modules is None:
                ...
                if self.config.encoder_prompt_config.position == "front" or \
                    ...
                    # logger.info(f"vis_attention_mask shape {vis_attention_mask.shape}\n" # 416 36
                    #             f"attention_mask shape {attention_mask.shape}") # 416 46
                    attention_mask = torch.cat(
                        [attention_mask[:, :input_length], vis_attention_mask, attention_mask[:, input_length:]],
                        dim=1)
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            attention_mask = _expand_mask(
                attention_mask, inputs_embeds.dtype, tgt_len=L
            )
    ...
    def forward(
        ...
        if encoder_hidden_states is not None and encoder_attention_mask is not None:
            ...
            encoder_attention_mask = _expand_mask(
                ...
                inputs_embeds.dtype,
                tgt_len=input_shape[-1] + self.config.decoder_preseqlen,
            )
...
class VLBartModel(BartModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if (
            self.config.encoder_prompt_config is not None
            and self.config.encoder_prompt_config.prompt_len > 0
        ):
            prefix_attention_mask = torch.ones(
...
class VLBart(BartForConditionalGeneration):
    ...
    def __init__(self, config: BartConfig):
        ...
        if adapter_config is not None:
            if getattr(adapter_config, "train_task_adapters", None) and getattr(
                adapter_config, "hypercomplex_adapters", None
            ):
                if adapter_config.shared_phm_rule:
                    ...
                    if self.factorized_phm_rule:
                        self.phm_rule_left = nn.Parameter(
                            torch.FloatTensor(phm_dim, phm_dim, 1),
                            requires_grad=adapter_config.learn_phm,
                        )
                        self.phm_rule_right = nn.Parameter(
                            torch.FloatTensor(phm_dim, 1, phm_dim),
                            requires_grad=adapter_config.learn_phm,
                        )
        ...
        def set_phm_rule(module):
            ...
            for name, sub_module in module.named_modules():
                if isinstance(sub_module, PHMLinear):
                    if self.factorized_phm_rule:
                        sub_module.set_phm_rule(
                            phm_rule_right=self.phm_rule_right,
...
if __name__ == "__main__":
    ...
    if (
        config.use_hyperformer
        or config.use_adapter
        or config.use_compacter
        or config.use_lradapter
    ):
        assert (
            config.use_hyperformer + config.use_adapter + config.use_compacter <= 1
        ), "You can only at most one kind of adapters."
        if config.use_hyperformer:
    ...
    generation_output = model.generate(
        **inputs, vis_inputs=(vis_feats, vis_pos), task="gqa"
    )
    ...
    print(
        f"trainable params/total params: {total_param}/{orig_param_size} = {total_param / orig_param_size * 100:.3f} %"
    )
