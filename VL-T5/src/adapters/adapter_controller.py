    ...
    def construct_adapters(self, tasks):
        ...
        if self.use_single_adapter:
            ...
            if self.share_up_sampler:
                layer = self.adapters[
                    tasks[0]
                ].up_sampler  # extract the layer of the adapters for first task
                for task in tasks:
            ...
            if self.share_down_sampler:
                layer = self.adapters[
                    tasks[0]
                ].down_sampler  # extract the layer of the adapters for first task
                for task in tasks:
                    self.adapters[task].down_sampler = layer

            if (
                self.hypercomplex_adapters
                and self.shared_phm_rule_over_tasks
                and not self.shared_phm_rule
            ):
