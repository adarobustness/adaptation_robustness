...
class AdapterLayersHyperNetController(nn.Module):
    ...
    def __init__(self, config, num_layers=6, include_cross_attention=False):
        ...
        self.feed_forward_up_sampler_hyper_net = AdapterLayersHyperNet(
            config, self.input_dim, self.down_sample_size
        )
        self.feed_forward_down_sampler_hyper_net = AdapterLayersHyperNet(
            config, self.down_sample_size, self.input_dim
        )
        self.self_attention_up_sampler_hyper_net = AdapterLayersHyperNet(
            config, self.input_dim, self.down_sample_size
        )
        self.self_attention_down_sampler_hyper_net = AdapterLayersHyperNet(
            config, self.down_sample_size, self.input_dim
        )
        ...
        if include_cross_attention:
            self.cross_attention_up_sampler_hyper_net = AdapterLayersHyperNet(
                config, self.input_dim, self.down_sample_size
            )
            self.cross_attention_down_sampler_hyper_net = AdapterLayersHyperNet(
                config, self.down_sample_size, self.input_dim
            )
    ...
    def get_embedding(self, task_embedding, layer_id):
        ...
        layer_id_tensor = torch.tensor(
            [layer_id], dtype=torch.long, device=task_embedding.device
        )
        layer_embedding = self.layer_id_embeddings(layer_id_tensor)
        layer_embedding = layer_embedding.view(-1)
        embeddings = torch.cat(
            [task_embedding.view(1, -1), layer_embedding.view(1, -1)], axis=0
        )
    ...
    def forward(self, task_embedding, layer_id):
        ...
        if self.include_cross_attention:
            cross_attention_down = self.cross_attention_down_sampler_hyper_net(
                embeddings
            )
            cross_attention_up = self.cross_attention_up_sampler_hyper_net(embeddings)
            cross_attention_output = AdapterOutput(
                up=cross_attention_up, down=cross_attention_down
            )
        else:
        ...
        if self.add_layer_norm_before_adapter:
            ...
            if self.include_cross_attention:
                ...
                cross_attention_output.pre_norm = LayerNormOutput(
        ...
        if self.add_layer_norm_after_adapter:
            ...
            if self.include_cross_attention:
                ...
                cross_attention_output.post_norm = LayerNormOutput(
                    weight=weight, bias=bias
                )
        return AdapterT5BlockOutput(
            feed_forward=feed_forward_output,
...
class AdapterLayersOneHyperNetController(nn.Module):
    ...
    def get_embedding(self, task_embedding, layer_id, block_type):
        ...
        type_id_tensor = torch.tensor(
            [block_type], dtype=torch.long, device=task_embedding.device
        )
    ...
    def forward(self, task_embedding, layer_id):
        ...
        if self.include_cross_attention:
            ...
            cross_attention_down = self.down_sampler_hyper_net(
