...
class JointEncoder(BartEncoder):
    ...
    def get_prompt(self, bsz, device):
        input_tokens = (
            self.encoder_prefix_tokens.unsqueeze(0).expand(bsz, -1).to(device)
        )  # (B, L)
...
class VLBartModel(BartModel):
    ...
    def get_prompt(self, bsz, device):
        input_tokens = (
            self.decoder_prefix_tokens.unsqueeze(0).expand(bsz, -1).to(device)
        )  # (B, L)
...
if __name__ == "__main__":
    ...
    generation_output = model.generate(**inputs, vis_inputs=(vis_feats, vis_pos))
