...
class InputPrompts(nn.Module):
    def __init__(self, config):
        ...
        # logger.critical(f"InputPrompts: prompt_len={self.prompt_len}, input_dim={self.input_dim}, mid_dim={self.mid_dim}")
    ...
    def get_prompt(self, bsz, device):
        ...
        prefix_prompt = self.prefix_embedding(
            input_tokens
        )  # (B, L, d_model * n_heads * n_layer)
