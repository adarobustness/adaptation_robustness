    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            for step_i, batch in enumerate(self.train_loader):
                ...
                if self.verbose:
                    ...
                    desc_str += (
                        f" TP {nlvr_results[0]} ({nlvr_results[0]/n_total*100:.1f}%)"
                    )
                    desc_str += (
                        f" FN {nlvr_results[1]} ({nlvr_results[1]/n_total*100:.1f}%)"
                    )
                    desc_str += (
                        f" FP {nlvr_results[2]} ({nlvr_results[2]/n_total*100:.1f}%)"
                    )
                    desc_str += (
                        f" TN {nlvr_results[3]} ({nlvr_results[3]/n_total*100:.1f}%)"
                    )
            ...
            if self.verbose:
                ...
                train_acc = (
                    self.train_loader.evaluator.evaluate_train(quesid2ans) * 100.0
                )
                ...
                for score_name, score in valid_score_dict.items():
                    wandb_log_dict[f"Valid/{score_name}"] = score * 100.0
        ...
        if self.verbose:
            ...
            for score_name, score in test_score_dict.items():
                wandb_log_dict[f"Test/{score_name}"] = score * 100.0
            wandb.log(wandb_log_dict, step=epoch)
