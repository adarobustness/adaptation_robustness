...
class JointEncoder(T5Stack):
    ...
    def forward(
        ...
        # logger.critical(f"inputs_embeds: {inputs_embeds.shape} before prompt")
        if self.prompt_modules is not None:
            prefix_embeds = self.prompt_modules(
                inputs_embeds.shape[0], inputs_embeds.device, task
            )
            # logger.critical(f"prefix_embeds: {prefix_embeds.shape}")
            inputs_embeds = torch.cat([prefix_embeds, inputs_embeds], dim=1)
        # logger.critical(f"inputs_embeds: {inputs_embeds.shape} after prompt")
