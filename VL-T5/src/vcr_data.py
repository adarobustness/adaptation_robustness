...
class VCRFineTuneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        object_tags = [
            tag.decode() if isinstance(tag, bytes) else tag for tag in object_tags
        ]
        ...
        if test:
            ...
            for i, rationale in enumerate(rationales):
                ...
                input_text = (
                    f"vcr qar question: {flat_q} answer: {flat_a} rationale: {flat_r}"
                )
                qar_input_texts.append(input_text)
    ...
    def collate_fn(self, batch):
        ...
        batch_entry["qa_input_ids"] = self.tokenizer(
            qa_input_texts,
            padding=True,
            truncation=True,
        ...
        batch_entry["qar_input_ids"] = self.tokenizer(
            qar_input_texts,
            padding=True,
            truncation=True,
            max_length=args.max_text_length,
        ...
        if not test:
            ...
            qa_target_ids = self.tokenizer(
                qa_target_texts, padding=True, return_tensors="pt"
            ).input_ids
            qar_target_ids = self.tokenizer(
                qar_target_texts, padding=True, return_tensors="pt"
            ).input_ids
...
def get_loader(
    ...
    dataset = VCRFineTuneDataset(
        split, rank=gpu, topk=topk, verbose=verbose, args=args, mode=mode
    )
