    ...
    def __init__(
        ...
        if self.adapter_type is not None:
            ...
            if "front" in adapter_pos:
                self.front_adapter = VisualAdapter(
                    inplanes, planes, adapter_kind, reduction_factor, use_bn
                )

            if "middle" in adapter_pos:
                self.middle_adapter = VisualAdapter(
                    planes, planes, adapter_kind, reduction_factor, use_bn
                )

            if "back" in adapter_pos:
                self.back_adapter = VisualAdapter(
                    planes,
            ...
            if "transition" in adapter_pos:
                self.transition_adapter = VisualAdapter(
                    planes * self.expansion,
                    planes * self.expansion,
                    adapter_kind,
                    reduction_factor,
...
class VisualAdapter(nn.Module):
    ...
    def __init__(
        self, input_dim, output_dim, adapter_kind, reduction_factor=16, use_bn=True
    ):
        super().__init__()
        ...
        if adapter_kind == "bottleneck":
            ...
            self.down_sampler = nn.Conv2d(
                input_dim, self.down_sample_size, 1, bias=False
            )
            self.up_sampler = nn.Conv2d(
                self.down_sample_size, output_dim, 1, bias=False
            )
...
if __name__ == "__main__":
    m = ModifiedResNet(
        ...
        input_resolution=224,
        width=64,
        adapter_type="front/back/middle-bottleneck",
        reduction_factor=26,
    )
