...
class Downsample(nn.Module):
    ...
    def forward(self, inputs_tuple):
        ...
        outputs = self.pool(
            inputs
        )  # (B, dim, self.output_size[0], self.output_size[1])
...
def main(params):
    ...
    if params["model_type"] != "vit_base_patch32_224_in21k":
        ...
        if "RN" in params["model_type"]:
            ...
            pos_embed = nn.Parameter(
                torch.zeros(
                    1,
                    num_patches + 1,
                    model.visual.attnpool.positional_embedding.shape[-1],
                    device="cuda",
                ),
            )
            pos_embed.weight = resize_pos_embed(
                model.visual.attnpool.positional_embedding.unsqueeze(0), pos_embed
            )
        ...
        pos_embed = nn.Parameter(
            torch.zeros(1, num_patches + 1, 768, device="cuda"),
        )
    ...
    if params["model_type"] == "ViT-B/32":
        num_patches = 196  # 600 * 1000 // 32 // 32
        pos_embed = nn.Parameter(
            torch.zeros(num_patches + 1, 768, device="cuda"),
        )
        pos_embed.weight = resize_pos_embed(
            model.visual.positional_embedding.unsqueeze(0), pos_embed.unsqueeze(0)
        )
    ...
    dir_att = os.path.join(
        params["output_dir"], "data_clip_" + save_model_type + "_att"
    )
    ...
    for i, img in enumerate(imgs):
        ...
        with torch.no_grad():
            try:
                image = preprocess(
                    Image.open(os.path.join(params["images_root"], img)).convert("RGB")
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--images_root",
        default="/playpen3/home/ylsung/data/train2014",
    ...
    parser.add_argument(
        ...
        default="RN50",
        type=str,
        help="RN50, RN101, RN50x4, ViT-B/32, vit_base_patch32_224_in21k",
    )
